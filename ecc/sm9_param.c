...
static void sm9_tryminusx(mpz_ptr q, mpz_ptr x)
{
    // 36x4 - 36x3 + 24x2 - 6x + 1
    //= ((36(x - 1)x + 24)x - 6)x + 1
    mpz_sub_ui(q, x, 1);
...
static void sm9_tryplusx(mpz_ptr q, mpz_ptr x)
{
    // 36x4 + 36x3 + 24x2 + 6x + 1
    //= ((36(x + 1)x + 24)x + 6)x + 1
    mpz_add_ui(q, x, 1);
...
static void sm9_cc_miller_no_denom(element_t res, mpz_t q, element_t P,
...
static void sm9_tateexp(element_t out)
{
    element_t x, y, epow;
    sm9_pairing_data_ptr p = out->field->pairing->data;
                        ...
                        pairing_t pairing)
{
    ...
    sm9_cc_miller_no_denom(out, pairing->r, in1, x, y, p->negalpha);
...
void pbc_param_init_sm9_gen(pbc_param_t p, int bits)
{
    sm9_init(p);
    sm9_param_ptr fp = p->data;
    ...
    for (;;)
    {
        ...
        mpz_sub(r, q, t);
        mpz_add_ui(r, r, 1);
        if (mpz_probab_prime_p(q, 10) && mpz_probab_prime_p(r, 10))
            break;
